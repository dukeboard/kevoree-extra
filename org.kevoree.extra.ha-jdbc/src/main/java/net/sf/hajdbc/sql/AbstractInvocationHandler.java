...
public abstract class AbstractInvocationHandler<Z, D extends Database<Z>, T, E extends Exception> implements InvocationHandler, SQLProxy<Z, D, T, E>
{
	...
	private <R> R invokeOnProxy(T object, Method method, Object[] parameters) throws E
	{
		...
		this.logger.trace("Invoking " + method.getName() + " using " + strategy.getClass().getName());
		...
		synchronized (this.objectMap)
		{
			...
			if (object == null)
			{
				...
				catch (Throwable e)
				{
					...
					{
						this.logger.warn( Messages.SQL_OBJECT_INIT_FAILED.getMessage(), this.getClass().getName(), database);
					}
				}
			}
	...
	protected void replay(D database, T object) throws E
	{
		synchronized (this.invokerMap)
		{
			...
			{
				this.logger.trace( "Replaying against database ");
		...
		public R createResult(SortedMap<D, R> resultMap)
		{
			...
			while (results.hasNext())
			{
				...
				if (!Objects.equals(primaryResult, result))
				{
					...
					{
						AbstractInvocationHandler.this.logger.error( Messages.DATABASE_INCONSISTENT.getMessage());
					}
				}
			}
