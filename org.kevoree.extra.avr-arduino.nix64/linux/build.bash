...
mkdir build
cd build
../configure --target=avr --prefix=$BINARY --disable-nsl --enable-languages=c,c++ --disable-libssp --disable-shared --with-gmp --with-mpfr --quiet
make  
# sometimes there are PATH issues with "sudo make install",
# so you might try "sudo su" and then "make install".
# be sure to 'exit' from the root account before continuing!
...
0' | make install  
if [ "$?" != "0" ]; then
echo "[FATAL ERROR]--> You may have to install ubuntu sudo apt-get install libgmp3-dev libmpfr-dev"
	exit -1
fi
...
../configure --build=`../config.guess` --host=avr --prefix=$BINARY --enable-languages=c,c++ --disable-shared --with-gmp --with-mpfr --quiet
make 
