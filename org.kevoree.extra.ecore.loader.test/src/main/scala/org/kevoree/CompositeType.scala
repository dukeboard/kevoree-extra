...
trait CompositeType extends KevoreeContainer with ComponentType {
		...
		def addAllChilds(childs : List[ComponentType]) {
		...
		def removeAllChilds() {
				this.childs = List[ComponentType]()
		}
		...
		def setWires(wires : List[Wire] ) {
				this.wires = wires
				wires.foreach{e=>e.eContainer = this}

		}

		def addWires(wires : Wire) {
				wires.eContainer = this
				this.wires = this.wires ++ List(wires)
		}

		def addAllWires(wires : List[Wire]) {
				wires.foreach{ elem => addWires(elem)}
		}
