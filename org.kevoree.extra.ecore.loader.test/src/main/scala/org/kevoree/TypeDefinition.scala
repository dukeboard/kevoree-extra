...
trait TypeDefinition extends KevoreeContainer with NamedElement {
		...
		def setDictionaryType(dictionaryType : DictionaryType ) {
				dictionaryType match {
						case l : DictionaryType => {
								this.dictionaryType = Some(dictionaryType)
						}
						case _ => this.dictionaryType = None
				}
				dictionaryType.eContainer = this
		...
		def addAllSuperTypes(superTypes : List[TypeDefinition]) {
				superTypes.foreach{ elem => addSuperTypes(elem)}
		}
		...
		def removeAllSuperTypes() {
				this.superTypes = List[TypeDefinition]()
		}
