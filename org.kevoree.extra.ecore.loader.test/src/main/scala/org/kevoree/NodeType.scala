...
trait NodeType extends KevoreeContainer with LifeCycleTypeDefinition {
		...
		def addAllManagedPrimitiveTypes(managedPrimitiveTypes : List[AdaptationPrimitiveType]) {
				managedPrimitiveTypes.foreach{ elem => addManagedPrimitiveTypes(elem)}
		}
		...
		def removeAllManagedPrimitiveTypes() {
				this.managedPrimitiveTypes = List[AdaptationPrimitiveType]()
		}
