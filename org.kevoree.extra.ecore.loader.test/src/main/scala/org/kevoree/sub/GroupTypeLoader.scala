...
trait GroupTypeLoader
		extends DictionaryType {

		def loadGroupTypes(parentId: String, parentNode: NodeSeq): Int = {
		...
		def resolveGroupTypes(parentId: String, parentNode: NodeSeq, owner: ContainerRoot) {
				val typeDefinitionsList = (parentNode \\ "typeDefinitions")
				var i = 0
				typeDefinitionsList.foreach { xmiElem =>
						val modelElem = ContainerRootLoadContext.map(parentId + "/@" + xmiElem.label + "." + i).asInstanceOf[GroupType]
