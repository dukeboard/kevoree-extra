...
trait ComponentTypeLoader
		extends DictionaryType
		with PortTypeRef
		with IntegrationPattern
		with ExtraFonctionalProperty {

		def loadComponentTypes(parentId: String, parentNode: NodeSeq): Int = {
		...
		def resolveComponentTypes(parentId: String, parentNode: NodeSeq, owner: ContainerRoot) {
				val typeDefinitionsList = (parentNode \\ "typeDefinitions")
				var i = 0
				typeDefinitionsList.foreach { xmiElem =>
						val modelElem = ContainerRootLoadContext.map(parentId + "/@" + xmiElem.label + "." + i).asInstanceOf[ComponentType]
