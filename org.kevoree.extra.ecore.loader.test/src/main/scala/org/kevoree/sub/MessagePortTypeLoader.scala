...
trait MessagePortTypeLoader
		extends DictionaryType {

		def loadMessagePortTypes(parentId: String, parentNode: NodeSeq): Int = {
		...
		def resolveMessagePortTypes(parentId: String, parentNode: NodeSeq, owner: ContainerRoot) {
				val typeDefinitionsList = (parentNode \\ "typeDefinitions")
				var i = 0
				typeDefinitionsList.foreach { xmiElem =>
						val modelElem = ContainerRootLoadContext.map(parentId + "/@" + xmiElem.label + "." + i).asInstanceOf[MessagePortType]
						...
						(xmiElem \ "@filters").headOption match {
								case Some(head) => {
										head.text.split(" ").foreach {
												xmiRef =>
														ContainerRootLoadContext.map.get(xmiRef) match {
