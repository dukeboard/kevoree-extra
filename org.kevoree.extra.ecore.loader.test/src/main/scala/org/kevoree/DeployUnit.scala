...
trait DeployUnit extends KevoreeContainer with NamedElement {
		...
		def addAllRequiredLibs(requiredLibs : List[DeployUnit]) {
				requiredLibs.foreach{ elem => addRequiredLibs(elem)}
		}
		...
		def removeAllRequiredLibs() {
				this.requiredLibs = List[DeployUnit]()
		}
		...
		def setTargetNodeType(targetNodeType : NodeType ) {
				targetNodeType match {
						case l : NodeType => {
								this.targetNodeType = Some(targetNodeType)
						}
