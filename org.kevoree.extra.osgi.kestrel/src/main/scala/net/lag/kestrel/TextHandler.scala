...
object TextCodec {
  ...
  */
  def apply(bytesRead: Int => Unit, bytesWritten: Int => Unit) = new Codec(read, write,    bytesRead, bytesWritten)
  ...
  val write = new Encoder[TextResponse] {
    def encode(resp: TextResponse) : Option[org.jboss.netty.buffer.ChannelBuffer] = {
      resp match {
        case response : TextResponse =>
          Some(response.toBuffer)
        case _ => None
      }
    }
  }
